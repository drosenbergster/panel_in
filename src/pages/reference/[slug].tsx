...
export default function ReferencePage({ compiledSource, frontmatter }: ReferencePageProps) {
  return (
    <>
      <SEOHead
        title={frontmatter.title}
        description={`${frontmatter.title}. Reference information for Oregon therapist credentialing.`}
        path={`/reference/${frontmatter.slug}`}
      />
...
export const getStaticPaths: GetStaticPaths = () => {
  const slugs = getAllContentSlugs('reference')
  return {
...
export const getStaticProps: GetStaticProps<ReferencePageProps> = async ({ params }) => {
  const slug = params!.slug as string
  const { source, frontmatter } = getContentBySlug('reference', slug)
